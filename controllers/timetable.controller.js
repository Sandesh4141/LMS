...
const createTimetableEntry = async (
  ...
  try {
    const result = await pool.query(
      "INSERT INTO Timetables (course_id, teacher_id, subject_id, day_of_week, start_time, end_time, room_number) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *",
      [
        course_id,
        teacher_id,
        subject_id,
...
const getTimetableByDay = async (day_of_week) => {
  try {
    const result = await pool.query(
      "SELECT * FROM Timetables WHERE day_of_week = $1 ORDER BY start_time",
      [day_of_week]
    );
    return result.rows;
  } catch (error) {
...
const getTimetableByTeacher = async (teacher_id) => {
  try {
    const result = await pool.query(
      "SELECT * FROM Timetables WHERE teacher_id = $1 ORDER BY day_of_week, start_time",
      [teacher_id]
    );
    return result.rows;
  } catch (error) {
